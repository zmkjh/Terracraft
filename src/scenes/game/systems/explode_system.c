...
static void handler(ztream_data_t data) {
    ...
    for (int i = 0; i < DESTROY_SPACE_NUM; i++) {
        ...
        if (space && *space != TNT && *space != ANIMAL) {
            *space = AIR;
        }
    }
    *tex = AIR;
...
void system_dependency_explode_system() {
    ztream_system_add_dependency(system_explode_system, system_listener);
    ztream_system_add_dependency(system_tex_block_system, system_explode_system);
}
